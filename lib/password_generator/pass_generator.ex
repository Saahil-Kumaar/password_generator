defmodule PassGenerator do
  @moduledoc """
  Generates random password depending on
  parameters, Module main functon is "generate(options)".
  ...
  defp validate_length_is_integer(true, options) do
    ...
    value =
      options_values
      |> Enum.all?(fn x -> String.to_atom(x)
      |> is_boolean() end)
  ...
  defp included_options(options) do
    Enum.filter(options, fn{_key, value} -> value
      |> String.trim()
